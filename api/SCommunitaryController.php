<?php 
    ...
    //Api-Rest: Report: Servicio Comunitario
	$app->group('/scommunitary', function(){
		$this->post('', function(Request $req, Response $res){
			try{
				$input = $req->getParsedBody();				
				$sql = "Insert into report(Id_Report_Type, Id_Career, Id_User, Report_Name, Students, isApproved, Observations) values (1,'".$input["Id_Career"]."','".$_SESSION["IdUser"]."','".$input["Report_Name"]."','".$input["Students"]."','".$input["isApproved"]."','".$input["Observations"]."');";
				$db = getConn();
				$db->query($sql);							
				$logsql = "Insert into user_log (Id_User, Date_Time,Event) values ('". $_SESSION["IdUser"] ."', '". date("Y-m-d H:i:s") ."','Ha registrado un informe de Servicio comunitario: ".$input["Report_Name"]."')";
				$db->query($logsql);
		...
		$this->get('', function(Request $req, Response $res){
			try{
				$sql = "Select Id_Report, Id_Report_Type, Id_Career, Id_User, Report_Name, Students, isApproved, Observations from report where Id_Report_Type = 1;";
				$db = getConn();
		...
		$this->get('/{id}', function(Request $req, Response $res){
			try{
				...
				$sql = "Select Id_Report, Id_Report_Type, Id_Career, Id_User, Report_Name, Students, isApproved, Observations from report where Id_Report = ".$id." and Id_Report_Type = 1;";
				$db = getConn();
				$statement = $db->query($sql);
				$data = $statement->fetchAll(PDO::FETCH_OBJ);
		...
		$this->patch('/{id}', function(Request $req, Response $res){
			try{
				...
				$db = getConn();
				$db->query($sql);							
				$logsql = "Insert into user_log (Id_User, Date_Time,Event) values ('". $_SESSION["IdUser"] ."', '". date("Y-m-d H:i:s") ."','Ha editado un informe de Servicio comunitario: ".$input["Report_Name"]."')";
				$db->query($logsql);
				if(isset($input["Archivos"])){
					foreach($input["Archivos"] as $file){
						if(isset($file["Base64"])){
		...
		$this->delete('/{id}', function(Request $req, Response $res){
			try{
				...
				$db = getConn();
				$db->query($sql);							
				$logsql = "Insert into user_log (Id_User, Date_Time,Event) values ('". $_SESSION["IdUser"] ."', '". date("Y-m-d H:i:s") ."','Ha borrado un informe de Servicio comunitario')";
				$db->query($logsql);
